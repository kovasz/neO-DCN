...
#		switchVars = solver.generateVars(self.numberOfSwitches + self.numberOfServers + 1)
		...
		# MIN = @SUM(EDGES: L);
		solver.minimize(list(linkVars.values()) + list(switchVars.values()))
...
#		outputVars = [[switchVars],[]]
		...
		outputVars["switches"] = [ switchVars[s] for s in range(1, self.numberOfServers + self.numberOfSwitches + 1) ]
		for l in self.links:
